...
public class Http2ConnectionHandler extends ByteToMessageDecoder
  ...
  private void doGracefulShutdown(
      ChannelHandlerContext ctx, ChannelFuture future, final ChannelPromise promise) {
    final ChannelFutureListener listener = newClosingChannelFutureListener(ctx, promise);
    if (isGracefulShutdownComplete()) {
      // If there are no active streams, close immediately after the GO_AWAY write completes or the
      // timeout
      // elapsed.
  ...
  protected void onStreamError(
      ChannelHandlerContext ctx,
      boolean outbound,
      @SuppressWarnings("unused") Throwable cause,
  ...
  private ChannelFuture resetUnknownStream(
      final ChannelHandlerContext ctx, int streamId, long errorCode, ChannelPromise promise) {
  ...
  private ChannelFuture resetStream(
    ...
    //
    // This needs to be done before we notify the promise as the promise may have a listener
    // attached that
  ...
  private ChannelFuture goAway(
      ChannelHandlerContext ctx, Http2Exception cause, ChannelPromise promise) {
  ...
  private void processRstStreamWriteResult(
      ChannelHandlerContext ctx, Http2Stream stream, ChannelFuture future) {
    if (future.isSuccess()) {
      closeStream(stream, future);
    } else {
  ...
  private static final class ClosingChannelFutureListener implements ChannelFutureListener {
    ...
    private void doClose() {
      // We need to guard against multiple calls as the timeout may trigger close() first and then
      // it will be
